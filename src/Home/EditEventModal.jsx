...
export default function EditEventModal(props) {
  ...
  const [showEditModal, setShowEditModal] = useState(
    props.stateValue.showEditModal
  );
  ...
  const [endTime, setEndTime] = useState(
    moment(props.event.end.dateTime).toDate()
  );
  ...
  function UpdateRecurring(event) {
    ...
    if (editRecurringOption === 'This event') {
        ...
        axios
          ...
          .then((res) => {
            ...
            for (let i = 0; i < res.data.items.length; i++) {
              if (res.data.items[i].id === props.event.id) {
                clickedEventIndex = i;
                break;
              }
            }
            ...
            let secondEventCount = res.data.items.length - clickedEventIndex;
      ...
      axios
        ...
        .then((res) => {
          console.log('/getRecurringEventInstances: ', res.data.items);
          parentEvent = res.data.items[0];
